...
AI pre-trained memories are static can hallucinate specific data points or citations. To help counteract this, Antigravity supports the Model Context Protocol (MCP),an open standard that allows you to securely connect your AI agent directly to live, external databases without needing to write custom API wrappers. For this workflow, add the MCP server [`arxiv-mcp-server`](https://github.com/blazickjp/arxiv-mcp-server) to Antigravity's settings. By explicitly routing the agent through an MCP server, you force it to ground its analysis in verified, retrieved full-text data rather than relying on a general web search or its pre-trained memory. This ensures maximum scientific rigor and reproducible data extraction.
